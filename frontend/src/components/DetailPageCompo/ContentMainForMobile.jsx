...
export default function ContentMainForMobile({ product }) {
  ...
  const mockProduct = {
    ...
    description: "This item is lightweight, stylish, and perfect for daily use. It features a powerful processor, long battery life, and high-resolution camera. Built with premium plastic material, supports fast charging, and runs on the latest OS.",
  };
  ...
  const handleAddToCart = () => {
    ...
    if (!p || p._id === "mock") {
      toast.error("Cannot add mock product to cart.");
      return;
    }
    if (p.stock <= 0) {
  ...
  return (
    <>
      ...
      <div className=" sm:hidden mb-5 flex flex-col w-full max-w-[1180px] h-auto container relative p-5 border border-gray-300 rounded bg-base-100  gap-5">
        ...
        {/* product name */}
        <h1 className="font-semibold text-[18px]">{p.name}</h1>
        {/* price */}
        <div className="flex items-center gap-2">
          <h1 className="text-[#FA3434] font-bold">${p.price}</h1>
        ...
        <table border="1" cellpadding="8" cellspacing="0">
          <tbody>
            <tr>
              <td className="text-gray-400">Stock</td>
        ...
        {/* desc */}
        <MobileDescription description={p.description} />
      </div>
      {/* suplire */}
